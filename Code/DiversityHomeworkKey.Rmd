...
```{r}
rm(list = ls() ) #fill in the code to clear R's memory for a fresh start
...
```{r}
DivIndices<-DF[, 1:3  ] #fill in the brackets to grab the 1st 3 columns
```

And make a dataframe that just has the tree species
Now combine those two into a third column to get unique plot IDS
```{r}
Trees<-DF[,4:24  ] #fill in the brackets to grab just the columns of tree species data but not sites
```
...
```{r}
ggsave(here("Data", "Outputs", "TreeDiversity.png"))
```
