...
```{r}
DF<-read.csv(here("Data", "Inputs","MammalMatrix.csv"))
```
...
```
Now make a plot that shows all the points per index and that makes the bottom 10% of values light red and the top 10% light green.

First, add a column to 
```{r}
...
```{r}
write.csv(Shared, here("Biodiv Lab", "Data", "Outputs","MammalDivComparison.csv"))
```
And write the diversity table out as well.
```{r}
write.csv(DivIndices, here("Biodiv Lab", "Data", "Outputs", "MammalDiversity.csv"))
```
